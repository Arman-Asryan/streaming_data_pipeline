...
if not os.path.exists(configs.log_folder):
    os.makedirs(configs.log_folder)
...
def download_from_yfinance(ticker, start_date, current_date, filename):
    ...
    data = yf.download(ticker, start=start_date, interval='2m')

    if data.empty:
        return {'stop': True, 'message': 'No data since {date} for {stock_code}'.format(date=start_date,stock_code=ticker)}

    data = data.reset_index(drop=False)
    data['Datetime'] = data['Datetime'].dt.strftime('%Y-%m-%d %H:%M:%S')
    data.rename(columns={'Adj Close':'Adj_Close', 'Datetime': 'Date_Time'}, inplace=True)
    data['ingestion_time'] = current_date.strftime('%H:%M:%S')
    data['ingestion_date'] = current_date.strftime('%Y-%m-%d')
    data['stock_code'] = ticker

    data = data.to_json(filename)
    return {'stop': False, 'message': 'Data downloaded since {date} for {stock_code}'.format(date=start_date,stock_code=ticker)}

# Upload data to Google Drive
def upload_from_local_to_drive(gauth_cred, client_config_file, folder_id, ticker, current_date, filename):
...
def add_data_to_raw_table(gauth_cred, client_config_file, folder_id, project_id, dataset_id, table_name, filename):
    gauth = GoogleAuth()
    ...
    pandas_gbq.to_gbq(df, f'{project_id}.{dataset_id}.{table_name}', project_id=project_id)



def load_query(query_name):
    for script in os.listdir(configs.queries):
        if query_name in script:
            with open(configs.queries + '\\' + script, 'r') as script_file:
